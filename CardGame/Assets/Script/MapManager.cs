...
{
    [SerializeField] int maxLevel = 0;  //纵向上层数
    [SerializeField] int maxCount = 0;  //横向每层最多房间数
    public int iSeed;                   //种子
    ...
    void ProcessThisLevel(int currentLevel)
    {
        ...
        int firstNum = 0;
        int secondNum = 0;
        int nextLevelLeftPoint = 0; //下一层的可连接点的左边界
        for (int i = 0; i < maxCount; i++)
        {
            if (mapNodes[currentLevel][i].isUsed)   //从第一个，即从上到下找第一个使用的节点
            {
                if (mapNodes[currentLevel + 1][i].isUsed || nextLevelLeftPoint >= i)    //如果下一层同一个位置的可以使用, 或者nextLevelLeftPoint大于等于该位置下标
                {
                    canLeft = false;
                }
                ...
                if (canLeft && canRight)
                {
                    firstNum = random.Next(3);      //0 1 2
                    secondNum = random.Next(3);
                }
                else if (canRight || canLeft)
                {
                    firstNum = random.Next(2);      //0 1
