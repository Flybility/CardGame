...
public class BezierArrow : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        RectTransformUtility.ScreenPointToLocalPointInRectangle(GetComponentInParent<RectTransform>(), Input.mousePosition, camera, out EndPoint);
        UpdateArrow(StartPoint, EndPoint);
    }
    ...
    public void Init()
    {
        ...
        for (int i = 0; i < NODE_COUNT - 1; i++)
        {
            ...
            Vector2 scale = new Vector2(0.2f + i / (float)(NODE_COUNT - 1) * 0.6f, 0.2f + i / (float)(NODE_COUNT - 1) * 0.6f);
    ...
    public void UpdateArrow(Vector2 startPos, Vector2 endPos)
    {
        ...
        ctrlAPos.y = endPos.y - (endPos.y - startPos.y) * 0.12f;
        ctrlBPos.y = endPos.y + (endPos.y - startPos.y) * 0.2f;
        ctrlBPos.x = startPos.x - (startPos.x - endPos.x) * 0.2f;
    ...
    private void UpdateAngle()
    {
        for (int i = 0; i < ArrowNodes.Count; i++)
        {
            ...
            else
            {
                ...
                if (ArrowNodes[i].transform.localPosition.x - ArrowNodes[i - 1].transform.localPosition.x > 0)
                {
                    angle = -angle;
                }
                //修改角度
